...
class SP_Layer_2():
    def __init__(self, config_filename='config.json', batch_size=40):
        ...
        self.train_dir = os.path.join(base_dir, config['layer2_train_dir'])
        self.test_dir = os.path.join(base_dir, config['layer2_test_dir'])
        self.checkout_dir = os.path.join(base_dir, config['checkout_l2'])
    ...
    def run_network(self, img_list=None):
        ...
        if is_test:
            ...
            print(len(train_imgs), len(train_vecs), len(test_imgs), len(test_vecs))
            for ei in range(500):
                # training
    ...
    def load_data(self):
        ...
        for tfile in train_files:
            ...
            t_vec_name = os.path.join(self.train_dir, tfile[:-3] + 'out')
            t_vec_file = open(t_vec_name, 'r')
            t_vec = np.load(t_vec_file)
            t_vec_file.close()
        ...
        for tfile in test_files:
            ...
            t_vec_name = os.path.join(self.test_dir, tfile[:-3] + 'out')
            t_vec_file = open(t_vec_name, 'r')
            t_vec = np.load(t_vec_file)
            t_vec_file.close()
...
def script_main():
    ...
    if len(sys.argv) > 1:
        if len(sys.argv) == 2 and sys.argv[1] == 'test':
            ...
            print('usage:')
            print('\tfor test  --> python SP_Layer_2.py test')
            print('\tfor train --> python SP_Layer_2.py')
            print('\n')
            return

    deep_symm = SP_Layer_2()
